#!/usr/bin/env bash
# ============================================================================
#  Haqiqat al-Qawmiyah Android · management console · WINDOWS
...
SRC_BOOK_DIR="${SRC_BOOK_DIR:-$HOME/koutoub/24-haqiqat-al-qawmiyah}"
KEYSTORE_DIR="${KEYSTORE_DIR:-$HOME/.keys}"
KEYSTORE_FILE="${KEYSTORE_FILE:-$KEYSTORE_DIR/wdiy-upload.keystore}"
APP_ID="${APP_ID:-org.workshopdiy.haqiqatalqawmiyah}"
AVD_NAME="${AVD_NAME:-haqiqat_al_qawmiyah_test}"
...
cmd_test() {
  ...
  local out="/tmp/haqiqat-al-qawmiyah-$(date +%Y%m%d-%H%M%S).png"
...
show_menu() {
  ...
  cat <<EOF
${BOLD}${MAGENTA}+==================================================================+
|   Haqiqat al-Qawmiyah Android . management console (Windows $SHELL_ENV)${RESET}${BOLD}${MAGENTA}
